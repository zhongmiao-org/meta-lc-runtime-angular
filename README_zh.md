...
`createSocketIoWebSocketManager` 会连接 BFF `/runtime` namespace，并把 `tenant.tenant-a.page.orders.instance.instance-1` 这类 runtime page topic channel 映射为 `subscribePage` 消息。测试或尚未启用实时传输的宿主仍可使用 `createDefaultWebSocketManager()`。
